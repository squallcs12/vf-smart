...
void registerOnboardingEndpoints(AsyncWebServer& server) {
  ...
  // Onboarding home page (redirect with current values and reconfig flag)
  ...
  server.on("/configure", HTTP_POST, [](AsyncWebServerRequest *request){
    ...
    if (ssid.length() < 1 || password.length() < 1 || api_key.length() < 8) {
      request->redirect("/error.html?message=Invalid%20configuration.%20API%20Key%20must%20be%20at%20least%208%20characters.&back=/");
      return;
    }
